window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    const zh = {
      ...
      quotaUnknown: "未知",
      quotaStale: "历史额度，点击“刷新全部额度”获取最新数据",
      quotaFailed: "额度刷新失败，当前为上次成功的数据",
      quotaNever: "尚未获取额度",
      quota: "额度",
      resetPrefix: "重置: {time}",
    ...
    const en = {
      ...
      quotaStale: "Cached quota. Refresh to get the latest data.",
      quotaFailed: "Quota refresh failed. Showing last successful data.",
      quotaNever: "Quota has not been fetched",
      quota: "Quota",
        ...
        quota.groups.map((group, gIdx) => {
          ...
          return React.createElement(
            ...
            buckets.map((bucket, bIdx) => {
              const fraction = typeof bucket.remainingFraction === "number" && Number.isFinite(bucket.remainingFraction) ? bucket.remainingFraction : null;
                ...
                React.createElement(
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      ...
                      percent === null ? tr("quotaUnknown") : `${percent}%`,
                    ),
                  ),
                ),
                React.createElement(
                  "div",
                  { className: "dshap-bar" },
                  percent === null ? null : React.createElement("div", {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement("div", { className: "dshap-card-title", style: { margin: 0 } }, `👥 ${tr("accountsTitle")} · v${poolStatus.version || "—"}`),
            React.createElement(
              "button",
              {
                className: "dshap-btn",
                ...
                return React.createElement(
                  ...
                  React.createElement("div", { style: { fontSize: "12px", color: "#6b7280", marginBottom: "8px" } },
                    account.quota?.fetchedAt
                      ? tr("updatedAt", { time: new Date(account.quota.fetchedAt).toLocaleString() })
                      : tr("quotaNever")),
                  account.quotaError || account.quotaStale
                    ? React.createElement("div", { role: "status", style: { color: "#92400e", marginBottom: "8px", fontSize: "12px" } },
                        tr(account.quotaError ? "quotaFailed" : "quotaStale"))
                    : null,
